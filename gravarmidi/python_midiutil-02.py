    ...
    for line in f:
        if line != '\n':
            if show:
                print(line)
            line = line.split()
            my_midi_file.append( {"track": int(line[0]), "pitch": int(line[1]), "dur": float(line[2]), "vol": round(float(line[3])*100) } )
...
def create_midi_file( mididata, outfile, type_write_midi ):
    ...
    MyMIDI = MIDIFile( total_track + 1 )      # Criar um track, formato padrao automatico
    ...
    for tk in range(0, total_track+1):
        ...
        for nota in mididata:
            if show:
                print( "Nota: ", nota["track"], i_time, nota["pitch"], nota["dur"], nota["vol"] )

            # add notas midi por track
            if type_write_midi != "total":
                if nota["track"] == int( type_write_midi ) :    # sempre sera 1 track por melodia criada
                    MyMIDI.addNote( 0, channel, nota["pitch"], time + i_time, nota["dur"], nota["vol"] ) # porcentagem para inteiro
                    i_time +=1
            else:
                if nota["track"] == tk :    # varios tracks
                    MyMIDI.addNote( nota["track"], channel, nota["pitch"], time + i_time, nota["dur"], nota["vol"] ) # porcentagem para inteiro
                    i_time +=1
